...
class PriceBot(discord.Client):
    ...
    async def update_price(self):
        ...
        # for digg include btc because mark to peg is important
        if self.token_display == "DIGG":
            activity_string += " btc=" + str(
                round(self.token_data.get("token_price_btc"), ndigits=3)
            )
        # for badger sett tokens, write different activity string for AUM
        if self.token_display[0] == "b":
            activity_string = "aum=$" + self._get_number_label(self._get_aum())
